...
import { X, Plus, User, Dumbbell, List, ChevronRight } from "lucide-react";
import { API_BASE_URL } from "@/utils/config";
import { Client, Exercise, ExerciseInPlan } from "@/types/workout";
...
export default function WorkoutPlans() {
  ...
  const fetchExerciseLists = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_BASE_URL}/api/trainer/exercise-lists`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleAddExercise = async (e: React.FormEvent) => {
    ...
    const selectedExercise = exercises.find(ex => ex.id === exerciseFormData.exerciseId);
    ...
    const newExercise: ExerciseInPlan = {
      ...
      weight: exerciseFormData.weight ? parseFloat(exerciseFormData.weight) : undefined,
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_BASE_URL}/api/trainer/exercise-lists/${selectedList.id}/exercises`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        setExerciseLists(exerciseLists.map(list => 
  ...
  const removeExercise = async (listId: string, exerciseIndex: number) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_BASE_URL}/api/trainer/exercise-lists/${listId}/exercises/${exerciseIndex}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    return (
      <div className="space-y-6 p-6">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Liste Esercizi Clienti</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {clients.map((client) => {
            ...
            return (
              <div key={client.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-700">
                <div className="p-6">
                    ...
                    {clientLists.length > 0 ? (
                      <>
                        <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                          ...
                          <span>{clientLists.length} lista{clientLists.length !== 1 ? 'e' : ''} di esercizi</span>
                        </div>
                        <button
    ...
    <div className="p-6">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400">Gestione Liste Esercizi</p>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 my-4">
        {clientLists.map((list) => (
          <div key={list.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700">
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-white">{list.name}</h3>
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto">
                {list.exercises.map((exercise, index) => (
                  <div key={index} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    <div>
                      <p className="font-medium text-gray-800 dark:text-white">{exercise.exerciseName}</p>
